...
# Examples: "Breakout", "FishingDerby", etc.
ENV_ID = "Breakout"     # <-- change to "Breakout" or others if you want
...
# Smaller replay buffer to avoid running out of RAM on DSMLP
BUFFER_SIZE = 1_000_000    # was 1_000_000
MIN_REPLAY_SIZE = 50_000   # was 50_000
...
def save_checkpoint(step, online_net, target_net, optimizer):
    """Save enough state to resume training later."""
    checkpoint = {
        "step": step,
...
def load_checkpoint(device):
    """Load checkpoint if it exists; return dict or None."""
    if not os.path.exists(CHECKPOINT_PATH):
        print("[Checkpoint] No existing checkpoint, starting from scratch.")
        return None
...
if __name__ == "__main__":
    ...
    # You could also use SubprocVecEnv instead of DummyVecEnv if needed:
    ...
    print("Warmup complete. Starting main training loop.")
    ...
    for step in range(start_step, TOTAL_STEPS):
        ...
        if step % CHECKPOINT_INTERVAL == 0 and step != 0:
            save_checkpoint(step, online_net, target_net, optimizer)
